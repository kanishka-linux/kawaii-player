...
**Note:** from `v7.0.0` web interface has undergone significant change as shown above.
...
*Note:* `ACCESS_FROM_OUTSIDE_NETWORK` and related settings are no longer supported. Please consider using vpn or tailscale like solution if user want to access media server outside of local network. Exposing server by port forwarding is not recommended. Following info on access from outside network only for legacy purpose and was available on very old versions, but no longer supported on new versions.
